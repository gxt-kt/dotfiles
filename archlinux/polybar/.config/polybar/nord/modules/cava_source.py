...
def KillProgram():
    for proc in psutil.process_iter(['name']):
        if all(s in proc.cmdline() for s in ['cava']):
            ...
            if "cava" in program and "polybar-cava-conf-source" in program :
                proc.kill()
...
if len(sys.argv) > 1 and sys.argv[1] == 'killandstop':
    common.ToggleState('cava source stop','true','false')
    KillProgram()
    sys.exit()

if len(sys.argv) > 1 and sys.argv[1] == 'toggle':
    common.ToggleState('cava source show','true','false')
    sys.exit()


# 启动前再检查下是否是停止状态
if(common.GetState('cava source stop','false')=="true"):
    if(common.GetState('cava source show','false')=="true"):
        print("STOP")
    else :
        print(" ❱ ")
    sys.exit()
...
if len(sys.argv) > 1 and sys.argv[1] == '--subproc':
    ...
    while True:
        ...
        if(common.GetState('cava source show','true')=="true"):
            print(output)
        else :
            print(" ❱ ")
...
default_source = re.search(r'Default Source:\s*(.*)', result.stdout).group(1)

cava_conf = tempfile.mkstemp('','polybar-cava-conf-source.')[1]
with open(cava_conf, 'w') as cava_conf_file:
    cava_conf_file.write(
       ...
       f'bars={opts.bars}\n'
        '[input]\n'
        'method=pulse\n'
        # 'source=alsa_input.usb-Razer_Razer_Nari_Ultimate-00.analog-chat\n'
